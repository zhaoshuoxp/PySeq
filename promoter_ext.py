...
def main():
	...
	parser.add_argument('-d','--distance', help = "extend (bp) from TSS (default 500)", default = 500, type = int)
	...
	lookup_bed(input_file,bed,outfile,distance)

def lookup_bed(i,b,o,d):
	genelist=[]
	...
	for line in open(b):
		...
		if a[4] in genelist:
			if a[3]=='+':
				f.writelines(a[0]+'\t'+str(int(a[1])-d)+'\t'+a[1]+'\t'+a[3]+'\t'+a[4]+'\n')
			elif a[3]=='-':
				f.writelines(a[0]+'\t'+a[2]+'\t'+str(int(a[2])+d)+'\t'+a[3]+'\t'+a[4]+'\n')
	f.close()
